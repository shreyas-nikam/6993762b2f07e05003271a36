...
# Streamlit Application Specification: AI Credit Model Fairness Audit

## 1. Application Overview

### Purpose of the Application

This Streamlit application serves as an interactive, step-by-step guide for CFA Charterholders and Investment Professionals, specifically Ms. Anya Sharma, CFA (Head of Model Governance), to conduct a comprehensive fairness audit of an AI-powered credit scoring model. It integrates advanced fairness testing techniques (group fairness metrics, proxy variable detection, counterfactual fairness, accuracy-fairness trade-off) into a user-friendly workflow, culminating in a regulatory-ready audit report. The app aims to demonstrate how to identify, quantify, and report on algorithmic bias, ensuring compliance with regulations like the ECOA and EU AI Act, while providing actionable insights for model governance.

### High-Level Story Flow of the Application

Ms. Anya Sharma, CFA, embarks on a crucial fairness audit for GlobalFin Bank's credit model. The application guides her through the following narrative steps:

1.  **Introduction & Setup**: Ms. Sharma first sets up her environment and loads the bank's pre-trained credit model and a simulated credit dataset. The app provides an overview of the model's baseline performance.
2.  **Augmenting Data with Synthetic Demographics**: Recognizing the sensitivity of protected attributes, Ms. Sharma uses the app to safely augment the credit data with *synthetic* demographic attributes (gender, race, age). This allows for bias detection without using real sensitive data in this demonstration, while understanding how such attributes can be legally incorporated in real-world audits.
3.  **Quantifying Group Fairness and Regulatory Compliance**: She then uses the augmented data to compute various group fairness metrics (e.g., Disparate Impact Ratio, Statistical Parity Difference) across these synthetic demographic groups. The app automatically checks for compliance with the "four-fifths rule," flagging potential disparate impact issues.
4.  **Identifying Potential Proxy Variables**: Ms. Sharma leverages SHAP analysis within the app to detect if seemingly neutral features are acting as "proxies" for protected attributes, inadvertently leading to indirect discrimination.
5.  **Conducting Counterfactual Fairness Tests**: To assess individual-level fairness, the app allows Ms. Sharma to perform counterfactual tests, exploring whether an individual's credit decision would change if only their protected attribute (and related proxies) were altered.
6.  **Analyzing the Accuracy-Fairness Trade-off**: A critical step involves quantifying the trade-off between model accuracy and fairness. The app compares the original model's performance to a fairness-constrained version, helping Ms. Sharma understand the "cost of fairness" and the implications of the "impossibility theorem."
7.  **Compiling a Regulatory-Ready Fairness Audit Report**: Finally, all findings and analyses are consolidated into a comprehensive, structured "AI Credit Model Fairness Audit Report." This report, generated within the app, summarizes findings, assesses severity, and outlines required actions, ready for presentation to legal, compliance, and senior management teams.

This flow empowers Ms. Sharma to move from raw data to a strategic, actionable report, making informed decisions to ensure ethical and compliant AI in finance.
...
from source import (
    ...
    compute_dir, # Helper function for trade-off page
    accuracy_fairness_tradeoff,
    plot_accuracy_fairness_tradeoff,
    compile_fairness_report
)
...
from sklearn.model_selection import train_test_split # Needed for initial data prep in session state
from sklearn.preprocessing import StandardScaler # Needed for initial data prep in session state
from xgboost import XGBClassifier # Needed for initial model in session state
from fairlearn.reductions import DemographicParity # Needed for trade-off page in session state
```

### Streamlit Application Structure and Flow

The application will be structured around a sidebar navigation menu (`st.selectbox`) to create a multi-page experience. `st.session_state` will be extensively used to store and pass data/results between these "pages".

#### Session State Initialization

Upon the first run of the application, `st.session_state` will be initialized with all necessary variables.
...
if 'X_test_fair' not in st.session_state:
    ...
    st.session_state.X_train_fair = None # Added for consistency with trade-off step
